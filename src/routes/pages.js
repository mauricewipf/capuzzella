 ...
 * @returns {Promise<{isPublished: boolean, hasUnpublishedChanges: boolean}>}
 */
...
router.get('/', async (req, res) => {
  try {
    ...
    // Sort pages: unpublished changes first, then unpublished drafts, then published
      ...
      <body class="bg-gray-100 min-h-screen">
        ...
        <script>
          ...
          async function unpublishPage(pagePath) {
            if (!confirm('Are you sure you want to unpublish this page? It will no longer be accessible to visitors.')) {
              return;
            }
